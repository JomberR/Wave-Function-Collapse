...
var _tile_nodes_to_be_erased: Array[Wave_Tile] = []

var collapsed_tile_id = -1
	...
	if(_is_adjacent(updated_cell_location)):
		for possible_tile in possible_tile_nodes:
			if(!possible_tile.is_valid(cell_superposition)):
				...
				_tile_nodes_to_be_erased.append(possible_tile)
				has_changed = true
				
	for erased_tile in _tile_nodes_to_be_erased:
...
#	random.randomize()
#
#	var rand_int = random.randi_range(0, possible_tile_nodes.size() - 1)
#	var collapsed_tile = possible_tile_nodes[rand_int]

	var collapsed_tile = _weighted_random_tile()
	
	collapsed_tile_id = collapsed_tile.source_id
...
func _place_tile():
	tileMap.set_cell(0, location, collapsed_tile_id, Vector2i(0,0))
	updated_tiles.emit(possible_tile_nodes, location)
...
func _weighted_random_tile() -> Wave_Tile:
	...
	for node in possible_tile_nodes:
		if(node.weight + offset >= rolled_value):
			return node
		else:
			offset += node.weight
	return null
